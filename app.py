...
import simulator.OpenBCIsimulation as bci_sim
...
class CcaLive(object):
    ...
    simulation : bool
        Set True if you'd like use OpenBCI Simulator.
    port : string
    ...
    def __init__(self, sampling_rate=256, connect=True, simulation=False,
                 port='', channels=[], path='', test_session=True, gui=False):
    ...
    def split(self, ref_signals):
        ...
        if self.simulation:
            self.board = bci_sim.OpenBCISimulator(self.PATH, self.CHANNELS, test=self.test_session)
            self.correlation = CrossCorrelation(self.sampling_rate,
                                                len(self.CHANNELS),
                                                self.ref)
            self.board.start_streaming(handle_sample)
            self.correlation.stats_all()
        else:
